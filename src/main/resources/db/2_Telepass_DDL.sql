...
--------------------------------------------------------
  ...
  CREATE TABLE TELEPASS.TB_CASELLO(
    ID_CASELLO_PK NUMBER(8,0) NOT NULL PRIMARY KEY,
	KM NUMBER(5,0),
	INGRESSI NUMBER(3,0),
...
--------------------------------------------------------
  CREATE TABLE TELEPASS.TB_TRANSPONDER(
    CODICE_TRANSP_PK VARCHAR(10) NOT NULL PRIMARY KEY,
...
--------------------------------------------------------
  ...
  CREATE TABLE TELEPASS.TB_VIAGGIO (
    TARGA_VE_PK VARCHAR(10) NOT NULL,
	CASELLO_ENTRY_FK_PK NUMBER(8,0) NOT NULL,
	TIME_ENTRY_PK DATE NOT NULL,
	CASELLO_EXIT_FK NUMBER(8,0),
	TIME_EXIT DATE,
	PEDAGGIO NUMBER(8,2),
	PAGATO_FLAG NUMBER(1,0) DEFAULT 0 NOT NULL,

    CONSTRAINT PK_VIAGGIO PRIMARY KEY (TARGA_VE_PK, CASELLO_ENTRY_FK_PK, TIME_ENTRY_PK),
    CONSTRAINT FK_TB_VIAGGIO_TB_VEICOLO FOREIGN KEY (TARGA_VE_PK) REFERENCES TB_VEICOLO(TARGA_PK) ON DELETE CASCADE,
    CONSTRAINT FK_TB_VIAGGIO_TB_CASELLO FOREIGN KEY (CASELLO_ENTRY_FK_PK) REFERENCES TB_CASELLO(ID_CASELLO_PK) ON DELETE CASCADE,
    CONSTRAINT FK_TB_VIAGGIO_TB_CASELLO_2 FOREIGN KEY (CASELLO_EXIT_FK) REFERENCES TB_CASELLO(ID_CASELLO_PK) ON DELETE CASCADE,
    CONSTRAINT CHECK_DATE_VIAGGIO CHECK (TIME_ENTRY_PK < TIME_EXIT),
